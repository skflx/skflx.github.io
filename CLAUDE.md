...
- **The JSON file is the database.** `data/kag-graph.json` (schema v2, ~656 nodes / ~929 edges) is the site's canonical knowledge graph — GitHub Pages is static, so there is no server. Contract: `docs/kag-schema.md` (authoritative; enums copied verbatim from the validator).
...
- **Plans live in `docs/`** (`oksat-plan.md`, `oksat-next-iteration-plan.md`, `oto-kag-atlas-plan.md`); read the relevant plan before large changes to OKSAT or the graph system.
